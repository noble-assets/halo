...
func TestAggregatorOwnerQuery(t *testing.T) {
	...
	res, err := server.Owner(goCtx, &aggregator.QueryOwner{})
...
func TestNextPriceQuery(t *testing.T) {
	...
	_, err := server.NextPrice(goCtx, nil)
	// ASSERT: The query should've failed due to invalid request.
	require.ErrorContains(t, err, errors.ErrInvalidRequest.Error())
...
func TestRoundDataQuery(t *testing.T) {
	...
	_, err := server.RoundData(goCtx, nil)
	// ASSERT: The query should've failed due to invalid request.
	require.ErrorContains(t, err, errors.ErrInvalidRequest.Error())
	...
	k.SetRound(ctx, 187, aggregator.RoundData{
		Answer:    sdk.NewInt(103780685),
		Balance:   sdk.NewInt(4791541000),
		Interest:  sdk.NewInt(701123),
		Supply:    sdk.NewInt(46169872257060),
		UpdatedAt: 1712071487,
	})

	// ACT: Attempt to query round data of unknown round.
	_, err = server.RoundData(goCtx, &aggregator.QueryRoundData{RoundId: 0})
	...
	res, err := server.RoundData(goCtx, &aggregator.QueryRoundData{RoundId: 187})
	// ASSERT: The query should've successfully returned round data.
	require.NoError(t, err)
	require.Equal(t, uint64(187), res.RoundId)
	require.Equal(t, sdk.NewInt(103780685), res.Answer)
...
func TestLatestRoundDataQuery(t *testing.T) {
	...
	_, err := server.LatestRoundData(goCtx, nil)
	// ASSERT: The query should've failed due to invalid request.
	require.ErrorContains(t, err, errors.ErrInvalidRequest.Error())
	...
	res, err := server.LatestRoundData(goCtx, &aggregator.QueryLatestRoundData{})
	// ASSERT: The query should've successfully returned round data.
	require.NoError(t, err)
	require.Equal(t, uint64(187), res.RoundId)
	require.Equal(t, sdk.NewInt(103780685), res.Answer)
...
func TestRoundDetailsQuery(t *testing.T) {
	...
	_, err := server.RoundDetails(goCtx, nil)
	// ASSERT: The query should've failed due to invalid request.
	require.ErrorContains(t, err, errors.ErrInvalidRequest.Error())
	...
	_, err = server.RoundDetails(goCtx, &aggregator.QueryRoundDetails{RoundId: 0})
	// ASSERT: The query should've failed due to unknown round.
	require.ErrorContains(t, err, "unknown round")

	// ACT: Attempt to query round details.
	res, err := server.RoundDetails(goCtx, &aggregator.QueryRoundDetails{RoundId: 187})
	// ASSERT: The query should've successfully returned round details.
	require.NoError(t, err)
	require.Equal(t, uint64(187), res.RoundId)
	require.Equal(t, sdk.NewInt(4791541000), res.Balance)
	require.Equal(t, sdk.NewInt(701123), res.Interest)
	require.Equal(t, sdk.NewInt(46169872257060), res.TotalSupply)
	require.Equal(t, int64(1712071487), res.UpdatedAt)
}
...
func TestLatestRoundDetailsQuery(t *testing.T) {
	...
	_, err := server.LatestRoundDetails(goCtx, nil)
	// ASSERT: The query should've failed due to invalid request.
	require.ErrorContains(t, err, errors.ErrInvalidRequest.Error())
	...
	// ACT: Attempt to query latest round details.
	res, err := server.LatestRoundDetails(goCtx, &aggregator.QueryLatestRoundDetails{})
	// ASSERT: The query should've successfully returned round details.
	require.NoError(t, err)
	require.Equal(t, uint64(187), res.RoundId)
	require.Equal(t, sdk.NewInt(4791541000), res.Balance)
